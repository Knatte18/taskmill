...
6. If a step fails: run `python ${CLAUDE_SKILL_DIR}/../../scripts/task_block.py <plan-file> "<reason>"` to mark it `[!]`, then block the backlog task via `python ${CLAUDE_SKILL_DIR}/../../scripts/task_block.py doc/backlog.md "<reason>"`.
...
8. If all steps complete: run `python ${CLAUDE_SKILL_DIR}/../../scripts/task_complete.py --delete doc/backlog.md`, then update `doc/changelog.md`.
